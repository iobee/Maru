...
enum IconWriter {
    ...
    static func run() throws {
        for icon in iconFiles {
            let image = makeAppIcon(size: CGFloat(icon.pixels))
    ...
    private static func writePNG(image: NSImage, to url: URL) throws {
        guard
            ...
            throw NSError(domain: "IconWriter", code: 1, userInfo: [NSLocalizedDescriptionKey: "Failed to encode PNG for \(url.lastPathComponent)"])
        }
